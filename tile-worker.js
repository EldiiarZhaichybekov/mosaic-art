...
self.onmessage=event=>{
  try {const optimized=OptimizedContour.generate(event.data.source),optimizedSource=OptimizedContour.asPhysicalInput(optimized);self.postMessage({id:event.data.id,result:TileLayout.generate(event.data.source,{...event.data.options,optimizedSource})});}
