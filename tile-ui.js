  ...
  function create({notify,onDetailed,onPhysical,getFormat,getImage,getCanvas,t}) {
    ...
    panel.innerHTML=`<div class="tile-tabs" role="group" data-i18n-attr="aria-label:tile.view" aria-label="${tr('view')}"><button id="tile-view-layout" aria-pressed="true"><span data-i18n="tile.layout">${tr('layout')}</span></button><button id="tile-view-plan" aria-pressed="false"><span data-i18n="tile.plan">${tr('plan')}</span></button></div>
      ...
      <div id="tile-editor" hidden><p class="tile-help"><span data-i18n="tile.editHelp">${tr('editHelp')}</span></p>
        ...
        <div class="tile-tabs"><button id="tile-apply"><span data-i18n="tile.apply">${tr('apply')}</span></button><button id="tile-delete"><span data-i18n="tile.delete">${tr('delete')}</span></button><button id="tile-add-large" data-i18n="tile.addLarge">${tr('addLarge')}</button><button id="tile-add-small" data-i18n="tile.addSmall">${tr('addSmall')}</button></div>
    ...
    function draw(ctx,layout,mount,selection=null,layer='final') {
      ...
      for(const t of layout.tiles){ctx.save();ctx.translate(t.xMm,t.yMm);ctx.rotate(t.angleDeg*Math.PI/180);ctx.fillStyle='#dc2626';ctx.fillRect(-t.lengthMm/2,-t.widthMm/2,t.lengthMm,t.widthMm);if(selection===t.id){ctx.strokeStyle='#1d4ed8';ctx.lineWidth=.5;ctx.strokeRect(-t.lengthMm/2,-t.widthMm/2,t.lengthMm,t.widthMm);}if(mount){ctx.fillStyle='white';ctx.font=(t.type==='small'?'1.8':'2.6')+'px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(t.id+'·'+t.lengthMm,0,0);}ctx.restore();}
      if(mount){ctx.fillStyle='#334155';ctx.font='3px sans-serif';ctx.textAlign='left';ctx.textBaseline='alphabetic';ctx.fillText(tr('mountHeader',{w,h,orientation:tr(layout.orientation),count:layout.tiles.length,large:T.inventory(layout.tiles).largeUsed,small:T.inventory(layout.tiles).smallUsed}),15,7);ctx.font='2.5px sans-serif';ctx.fillText(tr('mountFooter'),15,h-5);}
    }
    ...
    function refresh() {
      if(hybridEnabled){const d=doc?.layout.hybridDiagnostics;hybridDetails.textContent=d?JSON.stringify({planner:doc.layout.mode==='RESULT2_RAIL'?'disabled':doc.layout.mode==='DETERMINISTIC_FALLBACK'&&d.errorCode?'failed':'accepted',qa:d.qa?(d.qa.accept?'accepted':'rejected'):d.errorCode&&doc.layout.mode!=='DETERMINISTIC_FALLBACK'?'failed':'not_run',reason:d.errorCode,issues:d.contractFailure?.issues,planning_attempts:d.planning_attempts||d.contractFailure?.planning_attempts,validationHistory:d.validationHistory||d.contractFailure?.validationHistory,mode:doc.layout.mode,visualStatus:doc.layout.visualStatus,...(debugEnabled?{plan:d.plan,qa:d.qa,routeCoverage:doc.layout.routeCoverage,inventory:T.inventory(doc.layout.tiles)}:{}),repairs:d.repairs,timings:d.timings},null,2):'';}
      $('tile-orientation').hidden=$('tile-format').value!=='30x40';$('tile-plan-info').hidden=!mounting||!doc;
      for(const type of ['large','small']){$('tile-add-'+type).disabled=!doc||pending||T.inventory(doc.layout.tiles)[type+'Remaining']===0||T.inventory(doc.layout.tiles).totalRemaining===0;$('tile-add-'+type).textContent=tr(adding===type?'clickCanvas':type==='large'?'addLarge':'addSmall');}
      ...
      if(doc){const layout=doc.layout,check=T.validate(layout),orient=tr(layout.orientation);
        $('tile-count').textContent=tr('count',{count:layout.tiles.length,large:T.inventory(layout.tiles).largeUsed,small:T.inventory(layout.tiles).smallUsed});
        ...
        if(hybridEnabled&&layout.mode!=='RESULT2_RAIL')$('tile-status').textContent+=' '+tr(layout.mode==='DETERMINISTIC_FALLBACK'?'hybridFallback':layout.visualStatus==='AI_ACCEPTED'?'hybridAccepted':'hybridReview');
        if(mounting)$('tile-plan-table').innerHTML=`<table><thead><tr><th>№</th><th>${tr('pieceSize')}</th><th>${tr('x')}</th><th>${tr('y')}</th><th>°</th></tr></thead><tbody>`+layout.tiles.map(t=>`<tr><td>${t.id}</td><td>${t.lengthMm} × ${t.widthMm}</td><td>${t.xMm.toFixed(2)}</td><td>${t.yMm.toFixed(2)}</td><td>${t.angleDeg.toFixed(2)}</td></tr>`).join('')+'</tbody></table>';
    ...
    canvas.addEventListener('pointerdown',e=>{if(!editing||!doc)return;const p=point(e);
      if(adding){const nearest=doc.layout.target.map(path=>({path,...T.nearestOnPath(p,path.points)})).sort((a,b)=>a.distance-b.distance)[0];const t=T.makeTile(...p,nearest?.angle||0,nearest?.path.role||'skeleton',nearest?.path.id||0,adding);
        ...
        const check=doc.add(T.snap(t,doc.layout,e.altKey||!$('tile-snap').checked));adding=false;if(!check.valid)notify(message(check),'error');else select(doc.layout.tiles[doc.layout.tiles.length-1].id);refresh();return;}
      const found=doc.layout.tiles.map(t=>({t,d:(()=>{const a=t.angleDeg*Math.PI/180,dx=p[0]-t.xMm,dy=p[1]-t.yMm;return Math.abs(dx*Math.cos(a)+dy*Math.sin(a))<=t.lengthMm/2+2&&Math.abs(-dx*Math.sin(a)+dy*Math.cos(a))<=4?Math.hypot(dx,dy):Infinity;})()})).sort((a,b)=>a.d-b.d)[0];
    ...
    for(const type of ['large','small'])$('tile-add-'+type).onclick=()=>{if(!doc||!T.inventory(doc.layout.tiles)[type+'Remaining']||!T.inventory(doc.layout.tiles).totalRemaining)return;adding=adding===type?false:type;refresh();};
    ...
    $('tile-csv').onclick=()=>{if(!doc||!T.validate(doc.layout).valid){notify(tr('fixFirst'),'error');return;}download('mounting-coordinates.csv','id,xMm,yMm,angleDeg,type,lengthMm,widthMm,role\n'+doc.layout.tiles.map(t=>[t.id,t.xMm,t.yMm,t.angleDeg,t.type,t.lengthMm,t.widthMm,t.role].join(',')).join('\n'),'text/csv');};
