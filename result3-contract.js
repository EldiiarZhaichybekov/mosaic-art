...
if(type==='array'){if(v.length<s.minItems||v.length>s.maxItems)errors.push({path,keyword:'itemsCount',minimum:s.minItems,maximum:s.maxItems,actualLength:v.length});if(s.items)v.slice(0,200).forEach((x,i)=>visit(s.items,x,path+'/'+i));}
