...
(function(root){'use strict';
 ...
 function create({dialog,t,select,uploadedPreview,onSelect}){
  ...
  let mode='silhouette',category='all',chosen=null,page=1,generation=0,loading=false;
  ...
  function labels(){dialog.querySelector('h2').textContent=tr('title');search.placeholder=tr('search');search.setAttribute('aria-label',tr('search'));tabs.setAttribute('aria-label',tr('title'));categories.setAttribute('aria-label',tr('title'));cancel.textContent=t('ux.cancel');confirm.textContent=t('ux.select');tabButtons.silhouette.textContent=tr('silhouettes');tabButtons.photo.textContent=tr('photos');Object.entries(tabButtons).forEach(([key,b])=>b.setAttribute('aria-pressed',String(key===mode)));}
  ...
  function card(asset){const b=button('',()=>{chosen=asset.id;selected();});b.className='silhouette-card';b.dataset.value=asset.id;const title=root.AssetLibrary.localized(asset.title,lang());b.setAttribute('aria-label',title);
   if(asset.id==='custom'){const cv=$('canvas');cv.width=240;cv.height=150;cv.setAttribute('aria-hidden','true');uploadedPreview(cv);b.append(cv);b.disabled=select.querySelector('[value=custom]').disabled;if(b.disabled)b.append($('small','',t('ux.uploadFirst')));}
   else {const img=$('img');img.src=asset.thumbnailUrl;img.alt='';img.loading='lazy';img.decoding='async';img.width=480;img.height=300;img.onerror=()=>{b.disabled=true;img.hidden=true;b.append($('small','',tr('assetError')));if(chosen===asset.id){chosen=null;selected();}};b.append(img);}
  ...
  async function render(append=false){const request=++generation;if(!append){page=1;grid.replaceChildren($('p','',tr('loading')));grid.scrollTop=0;}loading=true;selected();labels();
   try{const [list,cats]=await Promise.all([repository.getAssets({type:mode,category,search:search.value,page,lang:lang()}),repository.getCategories(mode)]);if(request!==generation)return;
    categories.replaceChildren();const makeCategory=(id,title)=>{const b=button(title,()=>{category=id;chosen=null;render();});b.dataset.category=id;b.setAttribute('aria-pressed',String(category===id));categories.append(b);};makeCategory('all',t('ux.all'));for(const c of cats)makeCategory(c.id,local(c.title,lang()));if(mode==='silhouette')makeCategory('uploaded',t('ux.uploaded'));
    if(!append)grid.replaceChildren();grid.querySelector('.library-more')?.remove();for(const a of list.items)grid.append(card(a));
    if(mode==='silhouette'&&page===1&&['all','uploaded'].includes(category)&&(!search.value||select.querySelector('[value=custom]').textContent.toLowerCase().includes(search.value.toLowerCase())))grid.append(card({id:'custom',title:{[lang()]:select.querySelector('[value=custom]').textContent}}));
  ...
  async function accept(){if(loading||!chosen)return;const id=chosen;loading=true;selected();try{const asset=id==='custom'?{id,type:'silhouette'}:await repository.getAsset(id);if(!asset)throw Error('UNKNOWN_ASSET');dialog.close();await onSelect(asset);}catch(error){root.WorkspaceDialogs.error(tr('assetError'));console.warn('library_selection_failed',{assetId:id,reason:error.name});}finally{loading=false;selected();}}
  let debounce;search.oninput=()=>{clearTimeout(debounce);generation++;loading=true;chosen=null;selected();debounce=setTimeout(()=>render(),100);};dialog.addEventListener('close',()=>{clearTimeout(debounce);generation++;});
  return {search,open(type='silhouette'){mode=type;category='all';chosen=type==='silhouette'&&!select.selectedOptions[0]?.disabled?select.value:null;search.value='';render();},refresh(){labels();if(dialog.open)render();}};
 }
