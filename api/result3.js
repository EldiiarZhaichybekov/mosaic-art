...
function messages(body){
  ...
  if(body.phase==='plan')content.push({type:'text',text:JSON.stringify({AVAILABLE_RESULT2_PATH_IDS:body.context.paths.filter(p=>p.source==='result2').map(p=>p.id),AVAILABLE_RESULT1_RESTORE_IDS:body.context.paths.filter(p=>p.source==='result1').map(p=>p.id),contractRules:'Use ONLY listed source IDs. Do not return omissions: unselected IDs are derived by the application. Priorities are 0..1. All required fields must exist; no nulls or extra properties. Coordinate anchors are pairs in 0..1; do not invent anchor IDs.',minimalValidExample:C.planningExample(body.context)})});
