...
(async()=>{
  ...
  global.fetch=async()=>({ok:true,status:200,text:async()=>JSON.stringify({status:'completed',output:[{type:'message',role:'assistant',content:[{type:'output_text',text:JSON.stringify(wirePlan)}]}]})});
  ...
  global.fetch=async()=>({ok:true,status:200,text:async()=>JSON.stringify({status:'completed',output:[{type:'message',role:'assistant',content:[{type:'output_text',text:'{"arbitrary":"svg"}'}]}]})});response=await call({phase:'plan',context,images});assert.equal(response.value.error.code,'AI_SCHEMA_INVALID');assert.equal(response.value.error.planning_attempts,2);
