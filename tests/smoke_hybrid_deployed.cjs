// Explicit live production experiment. At most one planning and one QA call.
...
(async()=>{
  ...
  const request=async(phase,payload)=>{const start=Date.now(),response=await fetch(base+'/api/result3',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phase,...payload}),signal:AbortSignal.timeout(35000)});const data=await response.json();console.log(JSON.stringify({phase,status:response.status,requestId:data.requestId||data.error?.requestId,wallMs:Date.now()-start,serverMs:data.durationMs,errorCode:data.error?.code}));if(!response.ok)throw Object.assign(Error(),{code:data.error?.code||'HTTP_ERROR'});return data;};
  ...
  console.log(JSON.stringify({mode:result.layout.mode,visualStatus:result.layout.visualStatus,tiles:result.layout.tiles.length,calls:result.calls,errorCode:result.errorCode,timings:result.timings}));
