...
const fs=require('node:fs'),assert=require('node:assert/strict'),T=require('../tile-layout'),O=require('../optimized-contour');
(async()=>{
  const base=process.argv[2],file=process.argv[3];assert.ok(base&&file,'URL and synthetic fixture required');
  for(const name of ['tile-layout.js','tile-ui.js','tile-worker.js','tile-i18n.js','optimized-contour.js','optimized-ui.js','optimized-worker.js']){
  ...
  const optimized=O.generate(source);O.assertResult(optimized);assert.ok(O.exportSVG(optimized).includes('<path '));
