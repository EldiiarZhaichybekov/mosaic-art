...
(function(root){'use strict';
  function create({t,notify,onSelect}){
    ...
    function loadPrecomputed(data){if(!source||JSON.stringify(data.canvas)!==JSON.stringify(sheet))return false;root.OptimizedContour.assertResult(data);stop();result=structuredClone(data);failed=false;refresh();return true;}
    return {hide,loadPrecomputed,isActive:()=>active,sourceChanged(data,canvasSize){stop();hide();source=data;sheet=canvasSize.slice();result=null;failed=false;},reset(){stop();hide();source=null;result=null;},export(kind){if(!result||pending)return;try{root.OptimizedContour.assertResult(result);const a=document.createElement('a');a.download=`contour-optimized-${sheet.join('x')}mm.${kind}`;let url=null;if(kind==='svg'){url=URL.createObjectURL(new Blob([root.OptimizedContour.exportSVG(result)],{type:'image/svg+xml'}));a.href=url;}else{const out=document.createElement('canvas');out.width=sheet[0]*6;out.height=sheet[1]*6;const ctx=out.getContext('2d');ctx.scale(6,6);draw(ctx,result);a.href=out.toDataURL('image/jpeg',.95);}document.body.append(a);a.click();setTimeout(()=>{a.remove();if(url)URL.revokeObjectURL(url);},2000);notify(tr('exported'),'success');}catch(error){notify(tr('failure'),'error');console.error('optimized_export_failure',error);}}};
  }
